-- =====================================================
-- AGREGAR started_at A LA VISTA tasks_with_workers_v2
-- =====================================================
-- Necesario para mostrar y editar los tiempos de inicio y finalización
-- de las asignaciones de trabajadores
...
SELECT 
  ...
  -- Workers array as JSON (incluye contract_type, started_at y completed_at)
  COALESCE(
    json_agg(
      CASE 
        WHEN ta.id IS NOT NULL THEN
          json_build_object(
            ...
            'is_paid', ta.is_paid,
            'contract_type', ta.contract_type
          )
        ELSE NULL
      END
...
'Vista de tareas con información completa de trabajadores, apartamento, piso, torre y proyecto. Incluye IDs para filtrado jerárquico, contract_type, started_at y completed_at de cada trabajador.';
