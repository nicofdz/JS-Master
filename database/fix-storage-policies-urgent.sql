-- Script URGENTE para corregir las políticas de storage del bucket 'invoices'
...
-- 2. Eliminar TODAS las políticas existentes del bucket invoices
...
CREATE POLICY "Allow all authenticated users to upload invoices" ON storage.objects
FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'invoices');

CREATE POLICY "Allow all authenticated users to view invoices" ON storage.objects
FOR SELECT TO authenticated
USING (bucket_id = 'invoices');

CREATE POLICY "Allow all authenticated users to delete invoices" ON storage.objects
