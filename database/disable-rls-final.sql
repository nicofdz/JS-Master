-- =====================================================
-- DESACTIVAR RLS COMPLETAMENTE - VERSIÓN FINAL
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.apartments;
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.activity_templates;
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.apartment_activities;
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.teams;
...
SELECT 'RLS desactivado completamente - Ahora deberías poder crear pisos sin problemas!' as status;
