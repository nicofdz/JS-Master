-- =====================================================
-- ENFORCE COMPLETED TASK STATUS FOR PAYMENTS (V2)
-- Solo las tareas marcadas como 'completed' pueden ser pagadas
-- =====================================================
...
BEGIN
  ...
  -- 9. Actualizar income_tracking (si existe)
  IF EXISTS(SELECT 1 FROM income_tracking WHERE worker_id = p_worker_id) THEN
    UPDATE income_tracking
    SET 
      paid_amount = paid_amount + p_payment_amount,
      pending_amount = pending_amount - p_payment_amount,
      updated_at = now()
    WHERE worker_id = p_worker_id;
  ELSE
    INSERT INTO income_tracking (worker_id, paid_amount, pending_amount)
    VALUES (p_worker_id, p_payment_amount, 0);
  END IF;
...
COMMENT ON FUNCTION public.process_worker_payment_v2 IS 'Procesa pagos a trabajadores. SOLO permite pagar asignaciones de tareas que estén marcadas como completadas (tasks.status = ''completed'').';
